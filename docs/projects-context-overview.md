...
_Compiled 2026-09-22 from the git history, READMEs, docs, and source of each repo.
Commit counts cover all of Jesse's author identities (`jesse.caddell@wsu.edu`,
GitHub noreply, `jesse.c@nextwavedev.org`)._
...
- You ran the projects with real engineering process: issue/PR templates,
  ...
  architecture docs.
- Suggested framing: _"I use AI coding agents as a force multiplier. I own
  the architecture, specs, review, and verification."_ This is an
...
- _(Status sync automation is claimed once, in the Practicum section. See
  section 7. Don't repeat it here.)_
...
- The deep singles facets are _scaffolding_ that waits on live product
  metafield data
...
- JSON → YAML workflow generation and committing to repos are listed as
  _planned_ in the API README, even though the hub README checks off the
